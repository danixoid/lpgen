...
<html lang="ru">
  <head>
    ...
    <link href="{!! url('css/spectrum.css') !!}" rel="stylesheet">
    ...
    <link href="{!! url('js/redactor/redactor.css') !!}" rel="stylesheet">
  ...
  <body>
  	...
  	<div class="menu" id="menu">
  		...
  		<div class="main" id="main">
  			...
  			<h3><span class="fui-list"></span> Блоки</h3>
  			
  			<ul id="elements">
				<li><a href="#" id="all">Все блоки</a></li>
				  
			@foreach($sections as $section)
				<li><a href="" id="{!! $section->name !!}">{!! $section->name !!}</a></li>
			@endforeach
  			</ul>
			...
			<a href="#" class="toggle"><span class="fui-gear"></span></a>
  			...
  			<h3><span class="fui-windows"></span> Страницы</h3>
  			...
  			<div class="sideButtons clearfix">
  				<a href="#" class="btn btn-primary btn-sm btn-embossed" id="addPage"><span class="fui-plus"></span> Добавить</a>
  				<!-- <a href="#exportModal" data-toggle="modal" class="btn btn-info btn-sm btn-embossed disabled actionButtons"><span class="fui-export"></span> Опубликовать</a> -->
  			</div>
        {{--<hr>
        <div class="text-center xsProjectLInk" style="padding-left:20px;padding-right:20px;margin-bottom:30px;">
  		...
  		<div class="second" id="second">
			...
			<a href="#" class="hideSecond" id="hideSecond"><span class="fui-arrow-left"></span> СПРЯТАТЬ</a>
  		
  			<ul id="elements">
			@foreach($sections as $section)
				@foreach($section->t_blocks as $block)
					<li class="element {!! $section->name !!} ui-draggable ui-draggable-handle" style="display: none;">
					@if(!file_exists(public_path('elements/thumbs/' . $block->name . '.png')))
						<iframe scrolling="no" height="{!! $block->height*0.25 ?: 'auto' !!}"
							@if($block->sandbox) 
								src="{!! route('builder.content',$block->name) !!}"  
								data-sandbox="" data-loaderfunction="{!! $block->loader !!}"
							@else src="about:blank"
							@endif></iframe>
					@else
						<img src="{!! url('/elements/thumbs/' . $block->name . '.png') !!}" 
							data-srcc="{!! route('builder.content',['content'=>$block->name]) !!}"
							data-height="{{ $block->height }}"
							@if($block->sandbox) data-sandbox="" data-loaderfunction="{!! $block->loader !!}" @endif>
					@endif
					</li>
				@endforeach
			@endforeach
    	...
    	<div class="screen" id="screen">
    		...
    		<div id="frameWrapper" class="frameWrapper empty">
    			...
    			<div class="start" id="start">
    				<span>Создайте свою страницу, перетащив элементы на экран</span>
    			</div>
    		</div>
    ...
    <div id="styleEditor" class="styleEditor">
    	...
    	<h3><span class="fui-new"></span> Детали</h3>
    	...
    	<ul class="breadcrumb">
 		...
 		<ul class="nav nav-tabs" id="detailTabs">
 			<li class="active"><a href="#tab1"><span class="fui-new"></span> Стиль</a></li>
 		  	<li style="display: none;"><a href="#link_Tab" id="link_Link"><span class="fui-clip"></span> Ссылка</a></li>
 		  	<li style="display: none;"><a href="#image_Tab" id="img_Link"><span class="fui-image"></span> Картинка</a></li>
 		  	<li style="display: none;"><a href="#icon_Tab" id="icon_Link"><span class="fa fa-flag"></span> Иконка</a></li>
 		  	<li style="display: none;"><a href="#video_Tab" id="video_Link"><span class="fa fa-youtube-play"></span> Видео</a></li>
 		</ul><!-- /tabs -->
			...
			<!-- /tabs -->
 			<div class="tab-pane link_Tab" id="link_Tab">
   				...
   				<select id="pageLinksDropdown">
   					<option value="#">Выберите блок (для одностраничника)</option>
   				</select>
 			...
 			<div class="tab-pane imageFileTab" id="image_Tab">
   				...
   				<input type="text" class="form-control" id="imageURL" placeholder="Ввериде URL" value="">
   				
   				<p class="text-center or">
   					<span>или</span>
   				</p>
   				
   				<form id="imageUploadForm" action="{!! route('builder.iupload'); !!}">
   				{!! csrf_field() !!}
   				    	...
   				    	<div class="fileinput fileinput-new" data-provides="fileinput">
   							...
   							<div class="buttons">
   								<span class="btn btn-primary btn-sm btn-embossed btn-file">
   									<span class="fileinput-new" data-trigger="fileinput" ><span class="fui-image"></span>&nbsp;&nbsp;Выберите картинку</span>
 			...
 			<div class="tab-pane iconTab" id="icon_Tab">
 				...
 				<label>Выберите иконку ниже: </label>
    	...
    	<div class="margin-bottom-5">
    		<button type="button" class="btn btn-primary btn-embossed btn-sm btn-block" id="saveStyling"><span class="fui-check-inverted"></span> Применить</button>
    	</div>
    	
    	<div class="sideButtons clearfix">
    		<button type="button" class="btn btn-inverse btn-embossed btn-xs" id="cloneElementButton"><span class="fui-windows"></span> Клонировать</button>
    		<button type="button" class="btn btn-warning btn-embossed btn-xs" id="resetStyleButton"><i class="fa fa-refresh"></i> Сбросить</button>
    		<button type="button" class="btn btn-danger btn-embossed btn-xs" id="removeElementButton"><span class="fui-cross-inverted"></span> Удалить</button>
    	</div>
    ...
    <div id="hidden">
    	<iframe src="{!! route('builder.skeleton') !!}" id="skeleton"></iframe>
    </div>
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body text-center">
                  Страничка будет удалена  <br>
                  <strong>Вы уверены, что хотите удалить эту страницу и импортировать данные?</strong><br><br>
  ...
  <div class="modal fade" id="xsErrorModal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                  ...
                  <div class="col-sm-10">
                      <input type="text" class="form-control" name="title" id="title" placeholder="Название сайта" value="B-Apps LPGen">
                  </div>
              </div>

              <div class="form-group">
                  <label for="desc" class="col-sm-2 control-label">Описание сайта</label>
                  <div class="col-sm-10">
                      <textarea class="form-control" name="description" id="description" placeholder="Это поле попадет в мета Description" rows="2" ></textarea>
                  </div>
              </div>

              <div class="form-group">
                  <label for="keyw" class="col-sm-2 control-label">Ключевые слова</label>
                  <div class="col-sm-10">
                      <textarea class="form-control" name="keywords" id="keywords" placeholder="Вводите ключевые слова через запятую" rows="2" ></textarea>
                  </div>
              </div>

              <div class="form-group">
                  <label for="keyw" class="col-sm-2 control-label">Автор</label>
                  <div class="col-sm-10">
                      <textarea class="form-control" name="author" id="author" placeholder="Автор странички"></textarea>
                  </div>
              </div>
		...
		<div class="modal-dialog">
	    	<div class="modal-content">
	      		<div class="modal-body">
	      			...
	      			<p>
	      				Вы хотите очистить блок?
	      			</p>
	      			<p>
	      				Все внесенные вами изменения будут стерты.
	        		</p>
    ...
    <script src="{!! url('js/flatui-checkbox.js') !!}"></script>
    <script src="{!! url('js/flatui-radio.js') !!}"></script>
    <script src="{!! url('js/jquery.tagsinput.js') !!}"></script>
    <script src="{!! url('js/flatui-fileinput.js') !!}"></script>
    <script src="{!! url('js/jquery.placeholder.js') !!}"></script>
    <script src="{!! url('js/jquery.zoomer.js') !!}"></script>
    <script src="{!! url('js/application.js') !!}"></script>
    <script src="{!! url('js/spectrum.js') !!}"></script>
    <script src="{!! url('js/chosen.jquery.min.js') !!}"></script>
    <script src="{!! url('js/redactor/redactor.min.js') !!}"></script>
